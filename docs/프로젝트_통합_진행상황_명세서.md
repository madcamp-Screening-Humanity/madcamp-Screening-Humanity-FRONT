...
- ⚠️ `GET /api/chat/models` - 모델 목록 조회 (**미구현**, Backend `chat.py`에 라우트 없음)
...
- [x] 에러 처리 강화 ✅
- [x] **감독 모드** (2026-01-27): `director_note`, `current_speaker`, `format_persona_for_actor`(제4의 벽), `[배우 1:]/[배우 2:]` 파싱 ✅
- [x] **ContextManager 연동**: `get_summary(session_id, db)` → `format_persona_for_actor(..., summary=summary)` ✅
...
#### 3.3.1 Phase 5.2: 컨텍스트 절약 요약 기능 ✅ **부분 구현 완료** (2026-01-27)

**구현 위치**: `server-b/backend/app/services/context_manager.py`

**구현 완료**:
- [x] Redis (선택): `REDIS_URL`, 실패 시 비활성
- [x] `ContextManager` 구현: `get_summary`, `save_summary`, `summarize_dialogue`
- [x] 세션별 요약: Redis → DB(ChatSummary) → Memory
- [x] `app/models/summary.py` (ChatSummary), `main.py` create_all
- [x] `chat.py` 연동: `get_summary` → `format_persona_for_actor(..., summary=summary)`
- [x] `evaluation.py`: `summarize_dialogue` → `save_summary`

**미구현**:
- [ ] 토큰 수 계산 및 모니터링
- [ ] 자동 요약 트리거 (컨텍스트 80% 사용 시)
- [ ] 슬라이딩 윈도우 (최근 15-18턴 유지, 이전 턴 요약), `manage_context` in chat
...
#### 3.3.4 AI 생성 API ✅ **구현 완료** (2026-01-26)
...
- [x] **감독 모드** (2026-01-27): 2인 캐릭터 선택, 초기/자동 대화, 감독 중재 입력, 1턴=2메시지 ✅

**상세 구현 내역**: `docs/구현_상태_요약_2026-01-26.md`의 "ChatRoom API 연동 및 TTS 통합", "감독 모드 및 Redis/ContextManager" 섹션 참조
...
#### 4.3.1 Phase 5.2: 컨텍스트 절약 요약 기능 ✅ **부분 구현 완료** (2026-01-27)

**구현 위치**: `server-b/backend/app/services/context_manager.py`

**완료**: ContextManager, Redis(선택), ChatSummary, `chat.py` get_summary, `evaluation.py` 요약 저장.  
...
### 6.2 Phase 5.2: 컨텍스트 절약 요약 기능 ✅ **부분 구현 완료** (2026-01-27)
...
2. **Phase 5.4: Frontend 턴 제한 설정화** ✅ **완료** (1일)
   - [x] 턴 제한 설정화 (환경 변수 지원) ✅
   - [x] 세션 관리 추가 ✅
...
**구현 완료**:
- ✅ `app/services/context_manager.py` (ContextManager, Redis 선택, ChatSummary, chat/evaluation 연동)
- ✅ `app/api/tts.py`
...
| 버전 | 날짜 | 변경 내용 |
|------|------|----------|
| 1.12 | 2026-01-27 | 감독 모드 및 Redis/ContextManager 반영 - 감독 모드 (director_note, current_speaker, format_persona_for_actor, [배우 1/2]); Phase 5.2 부분 완료 (ContextManager, Redis, ChatSummary, chat/evaluation) |
| 1.13 | 2026-01-27 | Phase 5.3(동시 접속 제한) 계획·가이드·체크리스트·참조 전면 삭제 |
| 1.14 | 2026-01-27 | BACK/FRONT 코드 검토 반영 — GET /api/chat/models, style, animations, /api/characters/generate 미구현; client chat body director_note/tts_* 미전달; DB create_all character·summary |
