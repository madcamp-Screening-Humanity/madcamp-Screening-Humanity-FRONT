...
## 7. Gemini SDK (AI 생성 API용)

### 7.1 개요

**⚠️ 중요**: Gemini SDK는 채팅용 LLM(vLLM/Ollama)과는 별도로, **AI 생성 API** (캐릭터 상세 설정 자동 생성)에 사용됩니다.

**용도**:
- 캐릭터 상세 설정 자동 생성 (`POST /api/ai/generate/character-details`)
- 상황 분석 및 줄거리 생성 (`POST /api/ai/generate/story`)

**구현 위치**: `server-b/backend/app/api/ai.py`

### 7.2 Google Generative AI SDK

**패키지**: `google-generativeai` (`requirements.txt`에 추가됨)
...
| 버전 | 날짜 | 변경 내용 |
|------|------|----------|
| 1.4 | 2026-01-26 | 백엔드 Gemini SDK 도입 반영 - google-generativeai 패키지 추가, Safety Settings 및 JSON Mode 적용, 환경변수 기반 설정 (GOOGLE_API_KEY, GOOGLE_API_MODEL, GOOGLE_SAFETY_THRESHOLD), 참고 문서 추가 |
