...
- **주요 기능**:
  ...
  - `POST /api/ai/generate/character-details` - 캐릭터 상세 설정 자동 생성 (JSON). **참고**: Frontend `characterApi.generateCharacterDetails()`는 `POST /api/characters/generate`를 호출(경로 불일치). 연동 시 `/api/ai/generate/character-details`로 client 수정 필요.
...
- **주요 기능**:
  - `ChatRequest` 모델 확장:
    - `persona`: 페르소나 텍스트 (프론트엔드에서 전달); 감독 모드 시 `[배우 1: 이름]`/`[배우 2: 이름]` 형식
    - `session_id`: 세션 ID (선택, 자동 생성)
    - `character_id`: 캐릭터 ID (voice_id 조회용, 현재 미사용)
    - `scenario`: 시나리오 정보 (opponent, situation, background)
    - `director_note`: 감독의 연출/중재 지시 (감독 모드)
    - `current_speaker`: 감독 모드에서 현재 말할 캐릭터
  ...
  - 페르소나 전달 및 포맷팅:
    - 프론트엔드에서 `persona`, `scenario`, `session_id`, `character_id`, `director_note`, `current_speaker` 전달
    - `format_persona_for_actor()` 함수로 배우/역할극 형식 포맷팅 (제4의 벽, `director_note`, `summary` 반영)
    - 포맷팅된 프롬프트를 system 메시지로 변환하여 LLM에 전달
  - Character 조회 로직 제거:
...
- **주요 기능**:
  ...
  - 초기 메시지 생성:
    - 첫 대사 AI 생성, sample_dialogue 미사용
...
## 9. 감독 모드 및 Redis/ContextManager 구현 상세 (2026-01-27 반영)
...
- **구현 상태**: ✅ 완전 구현 완료
- **주요 기능**:
  - `ChatRequest.director_note`: 감독의 긴급/연출 지시
  - `ChatRequest.current_speaker`: 감독 모드에서 현재 말할 캐릭터
  - `format_persona_for_actor()`: 제4의 벽 유지 (감독은 투명인간), `director_note` 반영
  - 감독 모드 감지: `persona`에 `[배우 1:`, `[배우 2:]` 포함 시 자동 판별
  - `[배우 1: 이름]` / `[배우 2: 이름]` 파싱, 교대 응답 (`current_speaker` 또는 ai_msg_count 기반)

#### ✅ Frontend
- **`components/chat-room.tsx`**: 감독 모드 초기 대화, 자동 대화 이어가기, 감독 중재 입력(`[감독 중재]`), 턴 카운트(1턴=2메시지)
- **`components/character-select-modal.tsx`**: 감독 모드 시 2인 캐릭터 선택
- **`components/scenario-setup.tsx`**, **`components/script-preview.tsx`**: 감독 모드 연동
...
### 9.2 Phase 5.2: ContextManager + Redis ✅ **부분 구현 완료**

#### ✅ ContextManager (`app/services/context_manager.py`)
- **구현 상태**: ✅ 구현 완료
- **기능**:
  - **Redis** (선택): `redis` 패키지 있으면 `REDIS_URL`로 연결, 실패 시 조용히 비활성
  - **DB (ChatSummary)**: `get_summary`, `save_summary` — 영구 저장
  - **메모리**: Redis/DB 없을 때 `_memory_store` 폴백
  - `get_summary(session_id, db)`: Redis → DB → Memory 우선순위
  - `save_summary(session_id, summary, db)`: DB → Redis → Memory
  - `summarize_dialogue(messages, previous_summary)`: LLM으로 요약 생성

#### ✅ ChatSummary 모델 (`app/models/summary.py`)
- `session_id` (PK), `summary`, `updated_at`
- `main.py` lifespan에서 `summary` 모듈 import 및 `create_all` 포함

#### ✅ Chat API 연동
- `chat.py`: `summary = await context_manager.get_summary(session_id, db)` 후 `format_persona_for_actor(..., summary=summary)`에 전달

#### ✅ Evaluation API 연동
- `evaluation.py`: `get_summary` → `summarize_dialogue` → `save_summary`로 대화 종료 시 요약 저장

#### ⚠️ Phase 5.2 고급 (미구현)
- 슬라이딩 윈도우(최근 N턴 유지 + 이전 턴 요약) 및 80% 컨텍스트 트리거 자동 요약은 `chat.py`에 `manage_context` 형태로 미통합. 필요 시 `PHASE5_SETUP.md` 가이드 참고.

---

## 10. 다음 단계 (미구현 항목)

### 10.1 Phase 5.2 고급 (선택)

- [ ] 슬라이딩 윈도우 및 80% 컨텍스트 트리거를 `chat.py`에 `manage_context` 형태로 통합
...
- `madcamp-Screening-Humanity-FRONT/docs/감독_중재_기능_구현.md`, `감독_모드_턴_카운트_수정.md`
- **Gemini API 참고 문서**:
  - [Gemini API 사용해보기](https://velog.io/@dyd1308/Gemini-api-%EC%82%AC%EC%9A%A9%ED%95%B4%EB%B3%B4%EA%B8%B0)
  - [Gemini 모델 버전](https://ai.google.dev/gemini-api/docs/models?hl=ko#model-versions)
...
**작성자**: AI Assistant  
**최종 업데이트**: 2026-01-27  
**버전**: 3.3 (감독 모드, Redis/ContextManager 반영)

**변경 이력**:
- v3.3 (2026-01-27): 감독 모드 및 Redis/ContextManager 반영
  - 감독 모드: Backend director_note, current_speaker, format_persona_for_actor(제4의 벽), 감독 모드 파싱; Frontend chat-room, character-select, scenario-setup, script-preview, store, api
  - Phase 5.2 부분 완료: ContextManager, Redis(선택), ChatSummary, get_summary in chat, evaluation 요약 저장; 슬라이딩 윈도우/80% 트리거는 미통합
- v3.4 (2026-01-27): Phase 5.3(동시 접속 제한) 계획·가이드·체크리스트·참조 전면 삭제
- v3.5 (2026-01-27): BACK/FRONT 코드 검토 반영 — GET /api/chat/models, style, animations, /api/characters/generate 미구현; client chat body director_note/tts_* 미전달; /api/ai/generate/character-details 경로 불일치; DB create_all character·summary (DATABASE_SCHEMA)
...
- v2.1 (2026-01-26): 인증 제거 및 프롬프트 전달 수정 반영
  - `/api/chat` 인증 제거. **참고**: `GET /api/chat/models` 라우트 미구현 (Backend `chat.py`에 없음). `styleApi`/`animationApi`의 `/api/style/*`, `/api/animations`도 Backend 미구현, Frontend client에만 정의. **client `chat()` body**: `director_note`, `current_speaker`, `tts_enabled`, `tts_mode`, `tts_delay_ms`, `tts_streaming_mode` 미전달 (Backend는 수신·처리 지원).
